...
static void scl_low()
{
        *i2c_scl_port &= PIN_LOW(i2c_scl_pin);
}

static void scl_high()
{
        *i2c_scl_port |= PIN_HIGH(i2c_scl_pin);
}

static void sda_low()
{
        *i2c_sda_port &= PIN_LOW(i2c_sda_pin);
}

static void sda_high()
{
        *i2c_sda_port |= PIN_HIGH(i2c_sda_pin);
}

static void i2c_set_clock_freq(uint32_t freq_in_hz)
{
	i2c_clock_cycle_period = (1000000*(1/(2*freq_in_hz))); 
...
{
	sda_low();
	k_busy_wait(i2c_clock_cycle_period);
        scl_low();
}
...
{
	i2c_sda_port = i2c_line_setup(sda_port, sda_pin);
	i2c_scl_port = i2c_line_setup(scl_port, scl_pin);
	i2c_sda_pin = sda_pin;
	i2c_scl_pin = scl_pin;
	i2c_set_clock_freq(scl_freq);
	i2c_idr = (uint32_t*)((int)i2c_sda_port - (gpio_odr - gpio_idr)); //0x04
}
...
uint8_t i2c_byte_read()
{
        ...
        for(uint8_t i = 0; i<8; i++)
        {
                read_buffer |= (((* i2c_idr) << i2c_sda_pin) << (7 - i));
		k_busy_wait(i2c_clock_cycle_period);
                scl_high();
		k_busy_wait(i2c_clock_cycle_period);
                scl_low();
