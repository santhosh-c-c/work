...
static bool read_input_pin(uint8_t port, uint8_t pin)
{
	volatile uint32_t * address = calculate_address(GPIO_BASE_ADDRESS, port);
	address = ((uint32_t*)((int)address + IDR_OFFSET));
	return ((*address >> pin) & 1);		
}

void complementry_blink(uint8_t input_port, uint8_t input_pin, uint8_t output_port, uint8_t output_pin)
{
        clock_enable(rcc_ahb1enr, input_port); // Clock enable for input port
        clock_enable(rcc_ahb1enr, output_port); // Clock enable for output port
	gpio_mode_set(input_port, input_pin, input_mode); // Input mode
	gpio_mode_set(output_port, output_pin, output_mode); // Output mode
	while(1)
	{
		if(read_input_pin(input_port, input_pin))
		{
